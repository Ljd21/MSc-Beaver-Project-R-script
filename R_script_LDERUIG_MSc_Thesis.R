...
##GREEN TEA ANALYSIS
#Load Data
tea_k <- read.csv(file = "Green Tea Data.csv")
...
Bioassay_EC_mean_SE <- read.csv(file = "Bioassay_EC_mean_SE.csv")
head(Bioassay_EC_mean_SE)
Bioassay_EC_bar <- ggplot(Bioassay_EC_mean_SE, aes(x=Location, y=mean_k, fill=Location)) +
...
Bioassay_ED_mean_SE <- read.csv(file = "Bioassay_ED_mean_SE.csv")
head(Bioassay_ED_mean_SE)
Bioassay_ED_bar <- ggplot(Bioassay_ED_mean_SE, aes(x=Location, y=mean_k, fill=Location)) +
  geom_bar(stat="identity", alpha = 0.7) +
  scale_fill_grey() +
  theme_bw() +
  geom_errorbar( aes(x=Location, ymin=mean_k-SE_k, ymax=mean_k+SE_k), width = 0.4, colour="black", alpha=0.9, size=1.3) +
...
Bioassay_CD_mean_SE <- read.csv(file = "Bioassay_CD_mean_SE.csv")
head(Bioassay_CD_mean_SE)
Bioassay_CD_bar <- ggplot(Bioassay_CD_mean_SE, aes(x=Location, y=mean_k, fill=Location)) +
  geom_bar(stat="identity", alpha = 0.7) +
  scale_fill_grey() +
  theme_bw() +
  geom_errorbar( aes(x=Location, ymin=mean_k-SE_k, ymax=mean_k+SE_k), width = 0.4, colour="black", alpha=0.9, size=1.3) +
...
Bioassay_microbial <- read.csv(file = "Bioassay_microbial.csv")
head(Bioassay_microbial)
Bioassay_microbial_bar <- ggplot(Bioassay_microbial, aes(x=Location, y=mean_k, fill=Location)) +
  geom_bar(stat="identity", alpha = 0.7) +
  scale_fill_grey() +
  theme_bw() +
  geom_errorbar( aes(x=Location, ymin=mean_k-SE_k, ymax=mean_k+SE_k), width = 0.4, colour="black", alpha=0.9, size=1.3) +
  facet_grid(. ~ C_N + Bioassay) +
  labs(x = "Treatment", y = "k-microbial") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
Bioassay_microbial_bar

#Scatterplot for microbial breakdown 
C_N_Data <- read.csv(file = "C_N_Data.csv")
...
Edis_line <- ggplot(Bioassay_f_Edis, aes(x=Distance, y=k)) +
  geom_point(size = 2, aes(fill = Bioassay)) +
  geom_point(aes(color = Bioassay)) +
  geom_smooth(method = "lm",
              se = TRUE,
...
BOD_df_mean2 <- BOD_df_mean
BOD_df_mean2$Category <- factor(BOD_df_mean2$Category, levels = c("Sites 1-5", "Sites 6-10", "Sites 11-15"))
ggplot(BOD_df_mean2, aes(x=Category, y=Mean, fill=Treatment)) +
